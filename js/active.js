...
function showNotification(message) {
  ...
  if (toastTextEl) {
    ...
    console.error('Toast text element not found!');
    return;
  }
...
function addFavourite(event, bookId) {
  ...
  // $(`#itemId-${bookId} .action`).hide();
  // $(`#itemId-${bookId} .wishlist`).hide();
  
  $.ajax({
      url: 'api.php',
      method: 'GET',
      data: { function_name: 'add_favourite', book_id : bookId }
  });
}
...
function removeFavourite(bookId){
  $.ajax({
    url: 'api.php',
    method: 'GET',
    data: { function_name: 'remove_favourite', book_id : bookId },
...
function shuffleDivs(idValue = "sort-parentDiv") {
  const parent = document.getElementById(idValue); // Select the parent div
  if (!parent) {
    console.error(`Parent div with id "${idValue}" not found.`);
    return;
  }
  const children = Array.from(parent.children); // Get an array of child divs

  // Shuffle the array of child divs
